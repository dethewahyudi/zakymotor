...
<tr><td align="center" colspan="18" style=" border-bottom: double 1px solid #000 ;"><b>KWITANSI PEMBAYARAN</b></td>
         ...
         <tr>
          ...
          <td>Tenor</td><td >:</td><td>{{$tenor}} Bulan</td>
        </tr>
    ...
    ?>
       ...
       <thead style=" border-bottom: 1px solid #000 double;">
        ...
        <tr style="font-weight: bold;text-align: center;"><td colspan="3">Total</td>
          <?php if($over=="0"){?>
          <td></td>
          <?php }else{ ?>
          <td>{{ $over }}<?php } ?></td>
         ...
         <?php if($paid2=="0"){?>
          <td></td>
          <?php }else{ ?>
         <td>{{ $paid2 }}<?php } ?></td><td></td><td>Rp. {{ number_format($total,0,',',',') }}</td></tr>
  
         </thead>
         
        <tr><td style="height: 100px;" colspan="7"></td>
